          ...
          Settings
            ...
            <div className="grid grid-cols-1 gap-4">
                {protocols.map((protocol) => (
                    <div
                        key={protocol.id}
                        className={`rounded-xl border-2 p-6 transition-all hover:shadow-lg hover:shadow-brand/10 ${(statusColors as Record<string, string>)[protocol.status]}`}
                    >
                        <div className="flex items-start justify-between mb-4">
                            <div className="flex items-start gap-4 flex-1">
                                <div className="mt-1">{(statusIcons as Record<string, React.ReactNode>)[protocol.status]}</div>
